...
document.addEventListener("DOMContentLoaded", () => {
    ...
    `;
        ...
        function generatePalette(size) {
            return Array.from({ length: size }, () => randomColor());
        }
        ...
        function renderPalette(colors, containerId) {
                        ...
                        if (!Array.isArray(colors) || colors.length === 0) {
                                container.innerHTML = "<p style='color:#666;'>⚠️ No colors extracted.</p>";
                                return;
                        }
                        const html = colors.map(hex => {
                                const rgb = hexToRgb(hex);
                                const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b);
                                return `
                    <div class="palette-card">
        ...
        // Generate Button
        document.getElementById("randomPaletteBtn").onclick = function () {
            const sizeInput = document.getElementById("paletteSize").value;
            if (!sizeInput || parseInt(sizeInput) <= 0) {
                showToast("⚠️ Enter a valid palette size!", "warning");
                return;
            }
            const size = parseInt(sizeInput);
            currentPalette = generatePalette(size);
            renderPalette(currentPalette, "randomPaletteSwatches");
            showToast(`🎨 Palette of ${size} colors generated!`, "success");
        };

        // Shuffle Button
        document.getElementById("shufflePaletteBtn").onclick = function () {
            if (currentPalette.length === 0) return;
            currentPalette = currentPalette.sort(() => Math.random() - 0.5);
            renderPalette(currentPalette, "randomPaletteSwatches");
            showToast("🔀 Palette shuffled!", "info");
        };

        // Save Palette Button
        document.getElementById("savePaletteBtn").onclick = function () {
            if (currentPalette.length === 0) return;
            let saved = JSON.parse(localStorage.getItem("savedPalettes") || "[]");

            // Save exactly current palette (any size)
            saved.push([...currentPalette]);

            localStorage.setItem("savedPalettes", JSON.stringify(saved));
            renderSavedPalettes(saved);
            showToast(`💾 Palette with ${currentPalette.length} colors saved!`, "success");
        };

        // Render Saved Palettes
        function renderSavedPalettes(saved) {
            const html = saved.map(pal =>
                `<div class="palette-card">
            <div class="colors" style="display:flex; width:100%; height:100%;">
                ${pal.map(c => `<div style="flex:1; background:${c}"></div>`).join("")}
            </div>
            <div class="color-info">
                ${pal.length} colors
            </div>
        ...
        // Load Saved on Start (EMPTY by default)
        document.addEventListener("DOMContentLoaded", () => {
            localStorage.removeItem("savedPalettes");
            renderSavedPalettes([]);
        });

        // Random Palette Clear Button
        document.addEventListener("DOMContentLoaded", function () {
            document.getElementById("clearRandomPaletteBtn").onclick = function clearRandomPalette() {
                document.getElementById("randomPaletteSwatches").innerHTML = "";
                document.getElementById("savedPalettes").innerHTML = "";
                currentPalette = [];
                localStorage.setItem("savedPalettes", JSON.stringify([]));
                renderSavedPalettes([]);
                showToast("🧹 All palettes cleared!", "warning");
            };
        });

        // Export Buttons
        document.getElementById("exportCssBtn").onclick = function () {
            if (currentPalette.length === 0) return;
        ...
        document.getElementById("exportPngBtn").onclick = function () {
            ...
            link.click();
            showToast("🖼️ PNG exported!", "success");
        };
        /* 🎲 Random Palette Inspiration END (JS) */

  // Default page dikhana (Home ya pehla tool)
  document.addEventListener("DOMContentLoaded", () => {
    if (window.location.hash) {
      showPage(window.location.hash.substring(1));
    } else {
      showPage("home", false); // default page, URL hash update NA karo
    }
  });

  // Logo click ke liye function (URL clean karega)
  function goHome() {
    showPage("home", false);   // Home show karo bina hash ke
    history.pushState("", document.title, "/"); // ✅ URL clean karo (sirf domain)
  }

  // Generic page show function
